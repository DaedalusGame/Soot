...
public class EntityMuseModel extends ModelBiped {
    protected ModelRenderer leftWing;
    protected ModelRenderer rightWing;

    public EntityMuseModel() {
        this(0.0F);
    }

    public EntityMuseModel(float modelSize) {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entity) {
        ...
        EntityMuse entityMuse = (EntityMuse) entity;

        if (entityMuse.isCharging()) {
            this.bipedRightArm.rotateAngleX = 3.7699115F;
            this.bipedLeftArm.rotateAngleX = 3.7699115F; //hug
        }
