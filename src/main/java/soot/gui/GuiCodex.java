...
public class GuiCodex extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks){
        ...
        if (this.categoryIndex == -1){
            ...
            for (float i = 0; i < numResearches; i ++){
                ...
                Minecraft.getMinecraft().getTextureManager().bindTexture(category.texture);
                ...
                this.drawTexturedModalRect(-6, -80-12f*instRaise, (int) category.u+(selected ? 16 : 0), (int) category.v, 12, 12);
            ...
            if (this.researchPage == -1){
                    ...
                    if (r.ancestors.size() > 0){
                        for (int l = 0; l < r.ancestors.size(); l ++){
                            ...
                            for (float j = 0; j < stacks; j ++){
                                float coeff = (float)Math.pow((j+1.0f)/stacks,1.5f);
                                b.begin(GL11.GL_QUAD_STRIP, DefaultVertexFormats.POSITION_COLOR);
                                soot.util.RenderUtil.renderWavyEmberLine(b, basePosX+x1, basePosY+y1, basePosX+x2, basePosY+y2, 4.0f*coeff, new Color(255,64,16));
                                tess.draw();
                            }
                            GlStateManager.shadeModel(GL11.GL_FLAT);
                            GlStateManager.enableTexture2D();
                ...
                GlStateManager.color(1f, 1f, 1f, 1f);
                List<String> strings = ResearchBase.getLines(I18n.format("embers.research."+c.name+"."+r.name+".desc"), 152);
    ...
    public float getVert(float i, float f1, float f2){
        ...
        return Math.abs(10.0f*((1.0f-(Math.abs(i/80.0f))) * (float)(Math.sin(coeff*f1) + 0.4f*Math.sin(coeff*f2))));
    }
